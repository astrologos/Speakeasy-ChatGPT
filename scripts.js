...
async function handleSubmit(event) {
  ...
  if (prompt.length===0){
    prompt="Echo:'Hello! how can I assist you today?'"
  }

  if (button=='submit'){
    await chatSubmit(prompt);
  }
...
async function chatSubmit(prompt) {
  ...
  try {
    ...
    const response = await axios.post('https://api.openai.com/v1/chat/completions', {
      ...
      "messages": [{
        "role": "system",
        "content": "You are a helpful assistant. You will provide pretty short and non-verbose answers in a simple way."
      }, {
        "role": "user",
        "content": prompt
      }]
